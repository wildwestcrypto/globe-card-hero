...
const ProjectDetailPanel: React.FC<PanelProps> = ({ project, onClose }) => {
  ...
  return (
    <motion.div
      ...
      className="fixed top-0 right-0 h-full w-full max-w-md bg-brand-dark/80 backdrop-blur-lg shadow-2xl z-50 flex flex-col"
    >
      {/* --- THIS IS THE NEW IMAGE ELEMENT --- */}
      {/* It only renders if an imageUrl exists for the project */}
      {project.imageUrl && (
        <img
          src={project.imageUrl}
          alt={project.name}
          className="w-full h-48 object-cover" // Styles the image
        />
      )}
      
      {/* The padding is now on an inner div so the image can be full-width */}
      <div className="p-6 flex-grow flex flex-col overflow-y-auto">
        <div className="flex justify-between items-start mb-4">
          ...
          <button onClick={onClose} className="p-2 -mt-2 -mr-2 rounded-full hover:bg-white/10">
            <FiX size={24} />
          </button>
        </div>
